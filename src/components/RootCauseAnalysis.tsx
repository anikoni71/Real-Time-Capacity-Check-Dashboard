...
const DiagramWrapper = ({ title, icon: Icon, children }: any) => {
  ...
  const { isFullscreen: isFullScreen, toggleFullscreen: toggleFullScreen } =
  ...
  const captureFullChartImage = async (): Promise<string | null> => {
    ...
    // Physical DOM Expansion (Width only)
...
export default function RootCauseAnalysis({
  ...
  const { flags, averages, worstRow, minCapacityProcess } = useMemo(() => {
    ...
    const worstRow = scoreboards.reduce(
      (worst, curr) =>
        curr.efficiencyValue < worst.efficiencyValue ? curr : worst,
      scoreboards[0],
    );

    // Find lowest capacity process across the filtered processes
    const minCapacityProcess =
      processes && processes.length > 0
        ? processes.reduce(
            (min, p) => (p.capacity < min.capacity ? p : min),
            processes[0],
          )
        : null;
    ...
    machine: [
      {
        id: "m1",
        label: "Speed Loss",
        desc: `Operating at ${worstRow.todayPlanLcEfficiency}% vs 75% Baseline`,
      },
      {
        id: "m2",
        label: "Maintenance Alert",
        desc: "Mechanical macro-stop risk detected",
      },
      {
        id: "m3",
        label: "Tool Wear Trends",
        desc: "Needle breakages, folder adjustments needed.",
      },
    ],
    manpower: [
      {
        ...
        desc: `Missing ${Math.max(0, parseFloat(worstRow.obMp) - parseFloat(worstRow.todayPreMp))} Operators`,
      },
      {
        id: "mp2",
        label: "Line Layout Unbalancing",
        desc: `on Runday ${worstRow.runday}`,
      },
      {
        id: "mp3",
        label: "Skill Deficit",
        desc: "New operator in critical bottleneck.",
      },
    ],
    method: [
      {
        id: "mt1",
        label: "Method Gap",
        desc: `Plan Target ${worstRow.todayPlanLcTarget} is short of ${worstRow.lineTarget100} Pcs`,
      },
      {
        id: "mt2",
        label: "SMV Deviation",
        desc: `Pitch target variance on Style ${worstRow.style}`,
      },
      {
        id: "mt3",
        label: "Layout Constraints",
        desc: "Poor material flow between stations.",
      },
    ],
    material: [
      {
        id: "mat1",
        label: "Tack Time Overrun",
        desc: `${worstRow.presentTackTimeSec}s Actual vs ${worstRow.tackTimeSec}s Standard`,
      },
      {
        id: "mat2",
        label: "Quality Trap",
        desc: `Active Rework detected on Buyer ${worstRow.buyer} production`,
      },
      {
        id: "mat3",
        label: "Handling Time",
        desc: "Excessive time spent untying bundles.",
      },
    ],
  };
  ...
  const solutions = {
    machine: [
      {
        id: "s-m1",
        label: "Urgent Preventive Maintenance (PM)",
        desc: "for Line Equipment",
      },
      {
        id: "s-m2",
        label: "Recalibrate Limits",
        desc: `Recalibrate RPM limits for Style ${worstRow.style}`,
      },
      {
        id: "s-m3",
        label: "Machine Swap",
        desc: "Bring in standby machine for slow bottlenecks.",
      },
    ],
    manpower: [
      {
        id: "s-mp1",
        label: "Float Operators",
        desc: `Deploy ${Math.max(0, parseFloat(worstRow.obMp) - parseFloat(worstRow.todayPreMp))} Float Operators to Line`,
      },
      {
        id: "s-mp2",
        label: "Cross-Training",
        desc: `Initiate ${worstRow.style} Skill Matrix Cross-Training`,
      },
      {
        id: "s-mp3",
        label: "Pacing feedback",
        desc: "Increase digital target visibility.",
      },
    ],
    method: [
      {
        id: "s-mt1",
        label: "Yamazumi Re-balance",
        desc: `Yamazumi Re-balance to meet ${worstRow.lineTarget100} Pcs`,
      },
      {
        id: "s-mt2",
        label: "Element Shift",
        desc: `Shift critical elements for Style ${worstRow.style}`,
      },
      {
        id: "s-mt3",
        label: "Flow layout update",
        desc: "Move material staging closer to operator.",
      },
    ],
    material: [
      {
        id: "s-mat1",
        label: "Buffer sizing",
        desc: `Increase WIP buffering to handle ${Math.max(0, parseFloat(worstRow.presentTackTimeSec) - parseFloat(worstRow.tackTimeSec)).toFixed(1)}s variations`,
      },
      {
        id: "s-mat2",
        label: "Upstream QA Alert",
        desc: `Issue QA Alert to cutter for ${worstRow.buyer}`,
      },
      {
        id: "s-mat3",
        label: "Material Handler Aid",
        desc: "Assign handler to un-bundle pieces.",
      },
    ],
  };
  ...
  const bottleneckDiag = {
    machine: [
      ...
      {
        id: "bt-1",
        label: "Machine Tuning",
        desc: `Tuning needed for ${minCapacityProcess?.processName || "Unknown"}`,
      },
      {
        id: "bt-2",
        label: "RPM Settings",
        desc: "Check if RPM matches SMV requirements",
      },
    ],
    method: [
      // Process
      {
        id: "bp-1",
        label: "Cycle Time Gap",
        desc: `Cycle time exceeds takt time by ${minCapacityProcess?.diffr || 0}s`,
      },
      {
        id: "bp-2",
        label: "Work Breakdown",
        desc: "Review process for unnecessary non-value adding steps",
      },
    ],
    manpower: [
      // Operator
      {
        id: "bo-1",
        label: "Pacing Support",
        desc: `Operator: ${minCapacityProcess?.operatorName || "Unknown"} needs pacing support`,
      },
      {
        id: "bo-2",
        label: "Skill Matrix",
        desc: "Verify operator proficiency for this specific task",
      },
    ],
    material: [
      // Quality
      {
        id: "bq-1",
        label: "Handling Delay",
        desc: `Check handling time around ${minCapacityProcess?.processName || "Unknown"}`,
      },
      {
        id: "bq-2",
        label: "Incoming Parts",
        desc: "Ensure upstream quality is not causing rework here",
      },
    ],
  };
  ...
  const activeNodeDetails = () => {
    ...
    const dataList =
      type === "causes"
        ? lossCauses[branch as keyof typeof lossCauses]
        : type === "bottleneck"
          ? bottleneckDiag[branch as keyof typeof bottleneckDiag]
    ...
    return (
      <div
        className={`mt-6 p-4 rounded-lg border-l-4 ${isFlagged ? (isCauses ? "bg-red-50 border-red-500" : "bg-emerald-50 border-emerald-500") : "bg-gray-50 border-gray-400"}`}
      >
        <h4 className="font-bold text-lg mb-3 flex items-center gap-2 uppercase tracking-wide text-gray-800">
          ...
          Deep Dive: {theme[branch as keyof typeof theme].label}{" "}
          {isCauses ? (isBottleneck ? "Diagnostics" : "Causes") : "Solutions"}
        </h4>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
        <div
          className={`p-3 rounded-md flex flex-col ${flags.machine ? "bg-red-50 border border-red-200" : "bg-gray-50"}`}
        >
          <span className="text-xs font-semibold text-gray-500 uppercase">
            Avg Efficiency
          </span>
          <span
            className={`text-2xl font-bold ${flags.machine ? "text-red-600" : "text-gray-800"}`}
          >
        ...
        <div
          className={`p-3 rounded-md flex flex-col ${flags.manpower ? "bg-red-50 border border-red-200" : "bg-gray-50"}`}
        >
          <span className="text-xs font-semibold text-gray-500 uppercase">
            Manpower (Present / OB)
          </span>
          <span
            className={`text-2xl font-bold ${flags.manpower ? "text-red-600" : "text-gray-800"}`}
          >
            {averages.todayMp.toFixed(1)} / {averages.obMp.toFixed(1)}
          </span>
          {flags.manpower && (
            <span className="text-xs text-red-500 mt-1">Gap identified!</span>
          )}
        </div>

        <div
          className={`p-3 rounded-md flex flex-col ${flags.method ? "bg-red-50 border border-red-200" : "bg-gray-50"}`}
        >
          <span className="text-xs font-semibold text-gray-500 uppercase">
            Target (Plan / 100%)
          </span>
          <span
            className={`text-2xl font-bold ${flags.method ? "text-red-600" : "text-gray-800"}`}
          >
        ...
        <div
          className={`p-3 rounded-md flex flex-col ${flags.material ? "bg-red-50 border border-red-200" : "bg-gray-50"}`}
        >
          <span className="text-xs font-semibold text-gray-500 uppercase">
            IE Takt Time Tracker
          </span>
          <span
            className={`text-sm font-bold mt-1 leading-tight ${flags.material ? "text-red-600" : "text-gray-800"}`}
          >
            PLAN TAKT TIME: {(averages.tackSum / scoreboards.length).toFixed(1)}
            s<br />
            vs
            <br />
            ACTUAL TAKT TIME:{" "}
            {(averages.presentTackSum / scoreboards.length).toFixed(1)}s
          </span>
          {flags.material && (
        ...
        <div className="p-3 rounded-md flex flex-col justify-center bg-blue-50 border border-blue-200 text-blue-800">
          ...
          <p className="text-xs mt-1 text-blue-700 opacity-90">
            Showing localized RCA metrics for{" "}
            {scoreboards.length === 1
              ? `Line ${worstRow.line}`
              : "aggregate flow"}
            . Lowest process: {minCapacityProcess?.processName || "N/A"}
          </p>
        </div>
      </div>
